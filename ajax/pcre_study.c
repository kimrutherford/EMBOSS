...
/* @funcstatic set_start_bits *************************************************
**
** Create bitmap of starting chars
**
** This function scans a compiled unanchored expression and attempts to build a
** bitmap of the set of initial characters. If it can't, it returns FALSE.
** As time goes by, we may be able to get more clever at doing this.
**
** @param [r]  code [const uschar*] points to an expression
	       ...
	       BOOL utf8, const compile_data *cd)
{
    ...
    /* This next statement and the later reference to dummy are here
       in order to trick the optimizer of the IBM C compiler for OS/2
       into generating correct code. Apparently IBM isn't going to fix
       the problem, and we would rather not disable optimization (in
       this module it actually makes a big difference, and the pcre
       module can use all the optimization it can get). */
  ...
  while (try_next)
	...
	case OP_TYPEMINQUERY:
	    ...
	    /* Character class where all the information is in a bit
	       map: set the bits and either carry on or not, according
	       to the repeat count. If it was a negative class, and we
	       are operating with UTF-8 characters, any byte with the
	       top-bit set is a potentially valid starter because it
	       may start a character with a value > 255. (This is
	       sub-optimal in that the character may be in the range
	       128-255, and those characters might be unwanted, but
	       that's as far as we go for the moment.) */
...
** @return [pcre_extra*] pointer to a pcre_extra block,
